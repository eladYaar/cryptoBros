import { ICoinShort } from "../interfaces/ICoin";

export function populateCards(data: ICoinShort[]): void {
    let cardHtml = "";
...
export function disableBackBtn(disable: boolean = true): void {
    $("#btnPrevTop").prop("disabled", disable);
    $("#btnPrevBottom").prop("disabled", disable);
}


export function showForwardsBack(isShown: boolean = true): void {
    if (isShown) {
        $("#nextPrevDivTop").show();
...
export function searchIsLoading(isLoading: boolean): void {
    if (isLoading) {
